...
int test7 ()
{
    ...
    std::cout << "Testing to see if equality and inequality operators work\n";
    std::cout << "correctly when we compare two sequences." << std::endl;

    std::cout << "At the beginning we are going to attach " << first <<"\n";
    std::cout << "to both sequences." << std::endl;
    a.attach(first);
    b.attach(first);

    std::cout << "Then we compare if they are equal.";
    if(a != b){
    ...
    std::cout << "So far they are equal.\n";
    std::cout << "Now we insert the same two numbers to \n";
    std::cout << "both sequences and one extra number\n";
    std::cout << "to the second sequence." << std::endl;
    a.insert(second);
    a.insert(third);
    b.insert(second);
    b.insert(third);
    ...
    std::cout << "We test if the are different, which should be true.";
    if(a == b){
        std::cout << "... Failed." << std::endl;
        return POINTS[7]*(1/4);
    }
    ...
    std::cout << "Now we add the same element to the first sequence\n";
    std::cout << "to see if the equal operator works correctly.";
    a.insert(first);
    ...
    std::cout << "Now one last test before finishing the section.\n";
    std::cout << "We are going to test again if the sequences are\n";
    std::cout << "different by adding the same amount numbers but with\n";
...
int run_a_test(int number, const char message[], int test_function( ), int max)
{
    ...
    cout << message << " (" << max << " percent)." << endl;
    result = test_function( );
    if (result > 0)
    {
        cout << "Test " << number << " got " << result << " percent";
...
// **************************************************************************
// int main( )
//   The main program calls all tests and prints the sum of all percentages
...
int main( )
{
    ...
    cout << sum << " percent out of the " << POINTS[0];
    cout << " percent from this test program.\n";
